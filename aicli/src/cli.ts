...
async function startChatSession(options: CliOptions): Promise<void> {
  try {
    ...
    if (onboardingService.isFirstRun()) {
      const shouldOnboard = await onboardingService.promptOnboarding();
      if (shouldOnboard) {
        await onboardingService.runQuickOnboarding();
      } else {
        onboardingService.markOnboardingComplete();
        uiRenderer.renderInfo('Skipping setup. You can run "warp setup" anytime.\n');
      }
    }
...
async function runSetup(): Promise<void> {
  ...
  uiRenderer.renderSuccess('Setup complete! Run "aicli" to start chatting.');
}
