...
export class CommandHandler {
  async handleCommand(input: string): Promise<boolean> {
    ...
    try {
      switch (command.toLowerCase()) {
        ...
        case "mcp":
          await this.handleMCP(argsString);
          break;

        case "docs":
          await this.handleDocs(argsString);
          break;
  ...
  private async handleProvider(name: string): Promise<void> {
    ...
    const validProviders = ["ollama", "openai", "anthropic", "gemini", "glm"];
  ...
  private async handleIndex(): Promise<void> {
    uiRenderer.renderLoading("Indexing codebase...");
    try {
      await ragService.indexCodebase(process.cwd(), (msg: string) => {
        uiRenderer.renderLoading(msg);
      });
      uiRenderer.stopLoading();
      uiRenderer.renderSuccess("Codebase indexed successfully.");
    } catch (error: any) {
      uiRenderer.stopLoading();
      uiRenderer.renderError(`Indexing failed: ${error.message}`);
    }
  }
  ...
  private async handleMCP(args: string): Promise<void> {
    ...
    if (subcommand === "connect") {
      ...
      if (!name || !command) {
        uiRenderer.renderError("Usage: /mcp connect <name> <command> [args...]");
        return;
      }

      uiRenderer.renderLoading(`Connecting to MCP server ${name}...`);
      try {
        await mcpService.connect(name, command, cmdArgs);
        await toolService.registerMCPTools([name]);

        uiRenderer.stopLoading();
        uiRenderer.renderSuccess(`Connected to ${name} and registered tools.`);
      } catch (e: any) {
        uiRenderer.stopLoading();
        uiRenderer.renderError(`Failed to connect: ${e.message}`);
      }
      return;
    }
    ...
    if (subcommand === "list") {
      ...
      if (servers.length === 0) {
        uiRenderer.renderInfo("No connected MCP servers.");
      } else {
        uiRenderer.renderList(servers, false);
      }
      return;
    }

    uiRenderer.renderInfo("Usage: /mcp [connect|list]");
  }
  ...
  private async handleDocs(args: string): Promise<void> {
    ...
    if (!subcommand || subcommand === "help") {
      uiRenderer.renderInfo("Usage: /docs [view|refresh|init] [file]");
      uiRenderer.renderInfo("  view [design|changelog|changes|agent] - View documentation file");
      uiRenderer.renderInfo("  refresh - Regenerate all documentation");
      uiRenderer.renderInfo("  init - Initialize .aicli directory with default files");
      return;
    }
    ...
    if (subcommand === "init") {
      try {
        ...
        uiRenderer.renderSuccess("Initialized .aicli directory with default documentation files.");
      } catch (error: any) {
        uiRenderer.renderError(`Failed to initialize: ${error.message}`);
      }
      return;
    }

    if (subcommand === "refresh") {
      uiRenderer.renderLoading("Refreshing project documentation...");
      try {
        await docsService.ensureDocsDirectory();
        uiRenderer.stopLoading();
        uiRenderer.renderSuccess("Documentation refreshed successfully.");
      } catch (error: any) {
        uiRenderer.stopLoading();
        uiRenderer.renderError(`Failed to refresh: ${error.message}`);
      }
      return;
    }

    if (subcommand === "view") {
      const docType = rest[0] || "design";
      const validTypes = ["design", "changelog", "changes", "agent"];

      if (!validTypes.includes(docType)) {
        uiRenderer.renderError(`Invalid document type. Choose from: ${validTypes.join(", ")}`);
        return;
      }
      ...
      try {
        ...
        switch (docType) {
          case "design":
            content = await docsService.getDesignDoc();
            break;
          case "changelog":
            content = await docsService.getChangelog();
            break;
          case "changes":
            content = await docsService.getRecentChanges();
            break;
          case "agent":
            content = await docsService.getAgentRules();
            break;
        }

        if (!content) {
          uiRenderer.renderInfo(`No ${docType} documentation found. Use /docs init to create it.`);
          return;
        }

        uiRenderer.renderBox(`${docType.charAt(0).toUpperCase() + docType.slice(1)} Documentation`, content, {
          color: "cyan",
        });
      } catch (error: any) {
        uiRenderer.renderError(`Failed to read ${docType}: ${error.message}`);
      }
      return;
    }

    uiRenderer.renderError(`Unknown subcommand: ${subcommand}. Use /docs help for usage.`);
  }
}
